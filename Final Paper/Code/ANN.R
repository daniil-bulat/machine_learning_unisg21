...
NN_performance <- function(NN) {
  ...
  # MSE 
  MSE_train = sum((results.train$prediction - results.train$actual)^2)/nrow(results.train)
  ...
  # MSE 
  MSE_test = sum((results.test$prediction - results.test$actual)^2)/nrow(results.test)
  
  ## results
  Results    <- c("In-sample", "Out-sample")
  Accuracy   <- c(paste0(round(acc.train, 2)*100, "%"), paste0(round(acc.test, 2)*100, "%"))
  MSE        <- c(paste0(round(MSE_train, 2)*100, "%"), paste0(round(MSE_test, 2)*100, "%"))
  Misclassed <- c(misclas.train , misclas.test)
  FalsePos.  <- c(falPos.train, falPos.test)
  FalseNeg.  <- c(falNeg.train, falNeg.test)
